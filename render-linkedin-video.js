...
// 1. Script Definitions with Exact 60s Breakdown
const SEGMENTS = [
  {
    id: 'seg01',
    scene: 'home',
    title: 'Tổng quan',
    timeLabel: '0–5s',
    text: 'Đây không phải công cụ đăng bài LinkedIn. Đây là một hệ thống giúp biến một ý tưởng thành nội dung có chiến lược và đo được kết quả.'
  },
  {
    id: 'seg02',
    scene: 'campaign',
    title: 'Chiến dịch',
    timeLabel: '5–11s',
    text: 'Mọi thứ bắt đầu từ một luận điểm. AI Strategist phân tích nó thành mục tiêu, góc phản biện, câu hỏi nghiên cứu và hành trình nội dung.'
  },
  {
    id: 'seg03',
    scene: 'research',
    title: 'Nghiên cứu & R&D',
    timeLabel: '11–18s',
    text: 'AI tiếp tục tìm nguồn, phân loại luận điểm và mô phỏng nhiều vai trò để phản biện trước khi chúng ta đưa một quan điểm ra thị trường.'
  },
  {
    id: 'seg04',
    scene: 'map',
    ...
    timeLabel: '18–24s',
    text: 'Những phát hiện đã được kiểm chứng được chuyển thành các chuỗi bài có logic, và mỗi bài đều trace được về nguồn gốc của nó.'
  },
  {
    id: 'seg05',
    scene: 'strategy',
    title: 'Chiến lược',
    timeLabel: '24–31s',
    text: 'Mỗi thương hiệu có thông điệp cốt lõi, đối tượng, bằng chứng, trụ nội dung và nhịp xuất bản riêng.'
  },
  {
    id: 'seg06',
    scene: 'studio',
    ...
    timeLabel: '31–38s',
    text: 'Từ toàn bộ bối cảnh đó, hệ thống tạo prompt hoàn chỉnh để AI viết đúng chiến lược, thay vì tạo những bài rời rạc.'
  },
  {
    id: 'seg07',
    scene: 'posts',
    title: 'Bài viết',
    timeLabel: '38–52s',
    text: 'Bài viết sau đó đi qua workflow rõ ràng: bản nháp, duyệt, đặt lịch và xuất bản, với các giới hạn vận hành được kiểm soát.'
  },
  {
    id: 'seg08',
    scene: 'results',
    title: 'Kết quả & ROI',
    timeLabel: '52–58s',
    text: 'Và cuối cùng, GO4AI theo dõi từ lượt hiển thị, lượt xem hồ sơ, tin nhắn cho tới cuộc gọi và cơ hội kinh doanh.'
  },
  {
    id: 'seg09',
    scene: 'home',
    title: 'Closing',
    timeLabel: '58–60s',
    text: 'GO4AI LinkedIn App. Từ ý tưởng, đến nội dung, đến kết quả — trong một hệ thống.'
  }
];
...
async function main() {
  ...
  // Step 1: Synthesize High-Quality Male Voice for all 9 scenes
  ...
  for (let i = 0; i < SEGMENTS.length; i++) {
    ...
    const synthRes = await synthesizeVoiceSpeech({
      ...
      rate: '+18%',
      pitch: '+0Hz',
      outFilePath: outAudio,
    });

    s.durationSec = synthRes.durationSec;
    s.audioPath = outAudio;
    totalSpeechSec += synthRes.durationSec;
    audioList.push(outAudio);
    console.log(`✓ ${synthRes.durationSec}s`);
  }

  console.log(`\n✓ All 9 voice segments ready. Total speech duration: ${totalSpeechSec.toFixed(2)}s\n`);

  // Step 2: Assemble Continuous Master Voice Soundtrack (60.0s)
  console.log('🎵 Step 2: Assembling Master Soundtrack & Background Music...');
  ...
  if (fs.existsSync(bgmPath)) {
    console.log('  → Mixing voice with Tech Ambient BGM (auto-ducking at -18dB)...');
  ...
  // Step 3: Launch Playwright & Record 60s Live Screen
  ...
  // Step 4: Find WebM & Mux Final MP4
  const webmFiles = fs.readdirSync(tempRecordDir).filter(f => f.endsWith('.webm'));
  if (webmFiles.length === 0) {
    throw new Error('Playwright did not output a webm video file');
  }
  ...
  // Clean temp recording directory
  try { fs.rmSync(tempRecordDir, { recursive: true, force: true }); } catch {}

  console.log('\n===============================================================');
  console.log('🎉 VIDEO 60S GO4AI LINKEDIN OS HOÀN THÀNH XUẤT SẮC!');
  console.log('===============================================================');
